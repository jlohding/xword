...
class Crossword(object):
    ...
    def legend(self):
        ...
        across = sorted([x for x in self.current_word_list if x.down_across() == "across"], key=lambda x: x.row)
        across = [f"{(x.row, x.col)} - {x.clue}" for x in across]

        down = sorted([x for x in self.current_word_list if x.down_across() == "down"], key=lambda x: x.col)
        down = [f"{(x.row, x.col)} - {x.clue}" for x in down]

        return across, down

        # outStr = ''
        # for word in self.current_word_list:
        #     outStr += f"{word.col}, {word.row}, {word.down_across()}, {word.clue}"
        # return outStr
...
class Word(object):
    ...
    def __repr__(self):
        return f"Word({self.word}, {self.clue})"
